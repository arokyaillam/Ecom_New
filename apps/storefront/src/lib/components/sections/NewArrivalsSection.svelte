...
<section class="py-12 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
  ...
  {#if products.length === 0}
    <p class="text-[var(--color-text-secondary)] text-center py-8">No new arrivals yet. Check back soon!</p>
  {:else}
    <div class={cn('grid gap-4 sm:gap-6', colsClass)}>
      {#each products as product (product.id)}
        ...
        >
          <!-- New badge -->
          <span class="absolute top-2 right-2 z-10 bg-[var(--color-accent)] text-[var(--color-text)] text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-[var(--radius-md)]">
            New
          </span>
          ...
          <div class="relative overflow-hidden" style="aspect-ratio: 1/1;">
            ...
            {#if product.discount && product.discount !== '0'}
              <span class="absolute top-2 left-2 bg-[var(--color-primary)] text-white text-xs font-semibold px-2 py-0.5 rounded-[var(--radius-md)]">
                {#if product.discountType === 'Percent'}
