...
<section class="py-12 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto">
  ...
  {#if products.length === 0}
    <p class="text-[var(--color-text-secondary)] text-center py-8">No featured products available yet.</p>
  {:else}
    <div class={cn('grid gap-4 sm:gap-6', colsClass)}>
      {#each products as product (product.id)}
        <article
          class="group bg-[var(--color-surface)] rounded-[var(--radius-md)] border border-[var(--color-border)] overflow-hidden transition-shadow duration-300 hover:shadow-lg"
        >
          <!-- Image -->
